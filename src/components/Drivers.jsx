...
import { Phone, Clock, Search, AlertCircle, X, Pencil, Trash2 } from 'lucide-react'
...
const emptyForm = {
  name: '', license: 'C', status: 'available',
  phone: '', experience: '', hoursThisWeek: 0, assignedTruck: null,
}
...
export default function Drivers() {
  ...
  const handleSubmit = () => {
    ...
    if (editingId !== null) {
      setDrivers(prev => prev.map(d => d.id === editingId ? { ...d, ...form, experience: parseInt(form.experience) } : d))
    } else {
      setDrivers(prev => [...prev, { ...form, id: drivers.length + 1, experience: parseInt(form.experience) }])
    }
    handleClose()
  }
  ...
  const handleEdit = (driver) => {
    ...
    setForm({ name: driver.name, license: driver.license, status: driver.status, phone: driver.phone, experience: driver.experience, hoursThisWeek: driver.hoursThisWeek, assignedTruck: driver.assignedTruck })
    setShowModal(true)
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        {Object.entries(statusConfig).map(([key, cfg]) => (
          <div key={key} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
            ...
            <div className="text-2xl font-bold text-gray-800">{drivers.filter(d => d.status === key).length}</div>
          </div>
        ))}
      </div>
      ...
      <div className="flex gap-3">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Buscar conductor..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-9 pr-4 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
        <select value={filter} onChange={e => setFilter(e.target.value)} className="text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            {filtered.map(driver => {
              ...
              return (
                <tr key={driver.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-5 py-4"><span className="text-xs font-semibold bg-gray-100 text-gray-700 px-2 py-1 rounded">{driver.license}</span></td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4"><span className="text-sm text-gray-700">{driver.experience} años</span></td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-2">
                      <button onClick={() => handleEdit(driver)} className="text-gray-400 hover:text-blue-600 transition-colors" title="Editar">
                        <Pencil size={15} />
                      </button>
                      <button onClick={() => setConfirmDelete(driver)} className="text-gray-400 hover:text-red-500 transition-colors" title="Eliminar">
                        <Trash2 size={15} />
                      </button>
                    </div>
                  </td>
        ...
        <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:9999, padding:'24px'}}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h2 className="text-lg font-bold text-gray-800">{editingId !== null ? 'Editar conductor' : 'Añadir conductor'}</h2>
            ...
            <div className="px-6 py-5 space-y-4">
              <div>
                ...
                <input type="text" value={form.name} onChange={e => setForm({...form, name: e.target.value})} placeholder="Ej: Juan García López" className={`w-full text-sm border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.name ? 'border-red-400' : 'border-gray-200'}`} />
                {errors.name && <p className="text-xs text-red-500 mt-1">{errors.name}</p>}
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-medium text-gray-600 mb-1 block">Licencia *</label>
                  <select value={form.license} onChange={e => setForm({...form, license: e.target.value})} className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
              ...
              <div>
                ...
                <input type="text" value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} placeholder="+34 612 345 678" className={`w-full text-sm border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.phone ? 'border-red-400' : 'border-gray-200'}`} />
                {errors.phone && <p className="text-xs text-red-500 mt-1">{errors.phone}</p>}
              </div>
              <div>
                <label className="text-xs font-medium text-gray-600 mb-1 block">Años de experiencia *</label>
                <input type="number" value={form.experience} onChange={e => setForm({...form, experience: e.target.value})} placeholder="Ej: 5" min="0" className={`w-full text-sm border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.experience ? 'border-red-400' : 'border-gray-200'}`} />
            ...
            <div className="flex gap-3 px-6 py-4 border-t border-gray-100">
              ...
              <button onClick={handleSubmit} className="flex-1 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-lg transition-all">{editingId !== null ? 'Guardar cambios' : 'Añadir conductor'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {confirmDelete && (
        <ConfirmModal
          ...
          message={`¿Estás seguro de que quieres eliminar a ${confirmDelete.name}?`}
          onConfirm={() => { setDrivers(prev => prev.filter(d => d.id !== confirmDelete.id)); setConfirmDelete(null) }}
